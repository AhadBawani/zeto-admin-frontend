...
const Header = ({ reference }) => {
  ...
  const filteredData = Products.filter((item) => {
    return (
      ...
      item?.categoryId?.category?.toLowerCase().includes(query?.toLowerCase()) ||
      item?.productName?.toLowerCase()?.includes(query?.toLowerCase()) ||
      item?.subCategoryId?.subCategory?.toLowerCase()?.includes(query?.toLowerCase())
    );
  });
  useEffect(() => {
    if (user) {
        ...
        <div className="nav_center">
          ...
          {query.length > 0 ? (
            <div ref={wrapperRef} className="searchCard">
              ...
              <ul>
                {filteredData.length < 1 && <li>Your Search Does not match</li>}
                {filteredData
                  .map((item, i) => {
                    return (
                      <li
        ...
        <div ref={wrapperRef} className="nav_right">
          {user ? (
            <div className="userAfterLogin">
              ...
              >
                <span>{user?.username?.split(" ")[0].toUpperCase().slice(0,10)}</span>
