...
import { AllSellerAction, SellerAction, UserOrderAction } from "../Redux/Actions/Actions";
...
const Rating = ({ orderId, setOrderId }) => {
  ...
  const submitHandler = () => {
    if (user) {
      ...
      if (!obj?.orderId) toast.success("Order Id require");
      else if (!obj?.userId) toast.success("User Id require");
      else if (!obj?.rating) toast.success("Please select Star ");
      else if (!obj?.review) toast.success("Review Require");
      else {
        axios
      ...
      toast.error("To submit Review you will Have to login");
    }
  };
  return (
    <>
