...
const FoodHome = () => {
  ...
  const seller_detail = useSelector((state) => state?.Products?.seller_detail);
  const Products = useSelector((state) => state.Products?.Products) || [];
  const user = useSelector((state) => state?.User?.user);
  const userCart = useSelector((state) => state?.User?.usercart) || [];
  const [getUserCart, setGetUserCart] = useState();
  const foodProducts =
    Products?.filter((item) => item.mainCategory === "Food") || [];
            ...
            <section className="fh_gallery">
              <div className="fh_hero">
                <div className="fh_hero_img">
                  <img src={seller_detail?.photo} alt="" />
                  <div className="fh_seller_logo">
                    <img src={seller_detail?.photo} />
                  </div>
                </div>
                <h2>{seller_detail?.name}</h2>
              </div>
              <hr />
